...
public record UpdateCommandRequest(
        ...
        @Schema(example = "portfolio", description = "Categoria do comando.")
        String category,

        @Schema(example = "Lista as principais tecnologias e habilidades", description = "Descrição curta do comando.")
        String description,

        @Schema(example = "all", description = "Idioma do comando (pt, en, all).")
        String language,

        @Schema(example = "[\"React, TypeScript, Spring Boot, PostgreSQL\"]", description = "A resposta do comando (máximo 20 itens).")
